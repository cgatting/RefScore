...
def _lazy_load_ml_deps():
    global nltk, torch, yake, SentenceTransformer, pipeline, AutoTokenizer, AutoModelForSeq2SeqLM, cosine_similarity
...
class NLPProcessor:
    def __init__(self, settings: dict):
        ...
        try:
            ...
            logging.error(f"Error initializing keyword extractor: {e}")
    ...
    def _ensure_sentence_model(self) -> bool:
        if self.sentence_model is not None:
            return True
        try:
            self.sentence_model = SentenceTransformer(self.settings['model_settings']['sentence_model'])
            if torch.cuda.is_available():
                self.sentence_model = self.sentence_model.to(self.device)
            return True
        except Exception as e:
            logging.error(f"Error initializing sentence model: {e}")
            self.sentence_model = None
            return False

    def _ensure_summarizer(self) -> bool:
        if self.sum_model is not None and self.sum_tokenizer is not None:
            return True
        try:
            model_name = self.settings['model_settings']['summarizer_model']
            self.sum_tokenizer = AutoTokenizer.from_pretrained(model_name)
            self.sum_model = AutoModelForSeq2SeqLM.from_pretrained(model_name, use_safetensors=False)
            if torch.cuda.is_available():
                self.sum_model = self.sum_model.to(self.device)
            return True
        except Exception as e:
            logging.error(f"Error initializing summarizer model: {e}")
            self.sum_tokenizer = None
            self.sum_model = None
            return False

    def generate_summary(self, text: str, max_length: int = 50, min_length: int = 10) -> str:
        try:
            if not self._ensure_summarizer():
                return text[:max_length]
            inputs = self.sum_tokenizer(text, return_tensors="pt", max_length=1024, truncation=True)
            if torch.cuda.is_available():
...
class SourceRankingEngine:
    ...
    async def rank_top_sources(self, doc_text: str, rows: int, threshold: float, use_refine: bool, progress_cb: Optional[Any] = None) -> List[Dict[str, Any]]:
        ...
        if self.settings.get('privacy_settings', {}).get('cache_enabled', True):
...
class DocumentRefiner:
    ...
    async def refine_document(self, doc_text: str) -> str:
        ...
        if not doc_text or not doc_text.strip():
            self.view.update_progress(1.0, "No content to refine")
            return ""
        ...
        total = len(sentences)
        if total == 0:
            self.view.update_progress(1.0, "No sentences found")
            return ""
