...
import { GuidedFixOverlay } from './GuidedFixOverlay';
...
interface DocumentViewerProps {
  ...
  finderOverride?: CitationFinderService;
}

// Matches \cite{}, \parencite{}, \textcite{} including arguments like [p. 1]
const CITATION_SPLIT_REGEX = new RegExp('(\\\\(?:cite|parencite|textcite|footcite)[a-zA-Z]*\\*?(?:\\[[^\\]]*\\])*\\{[^}]+\\})');

export const DocumentViewer: React.FC<DocumentViewerProps> = ({ result, onReset, onUpdate, manuscriptText, bibliographyText, scoringConfig, finderOverride }) => {
  ...
  const [gapRecommendations, setGapRecommendations] = useState<Record<number, ProcessedReference[]>>({});
  const [gapPinned, setGapPinned] = useState<Set<number>>(new Set());
  const [gapResolved, setGapResolved] = useState<Set<number>>(new Set());
  const [gapStatus, setGapStatus] = useState<string>('');
  
  // Re-create service when config changes
  const finderService = React.useMemo(() => finderOverride ?? new CitationFinderService(scoringConfig), [scoringConfig, finderOverride]);
  ...
  const panelContent = activeAnalysis ? (
              ...
              <div className="flex items-baseline gap-1">
                <span
                  className={`text-4xl font-display font-bold tracking-tighter ${
                    computeScore(activeAnalysis.scores) >= 70
                      ? 'text-brand-400'
                      : computeScore(activeAnalysis.scores) >= 50
                      ? 'text-slate-200'
                      : 'text-red-400'
                  }`}
                >
                      ...
                      <div key={source.id || idx} className="p-4 bg-brand-500/10 border border-brand-500/20 rounded-xl">
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold text-brand-300 bg-brand-500/20 px-2 py-0.5 rounded-full border border-brand-500/20">
                              Option {idx + 1}
                            </span>
                            <span
                              className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                                score >= 70
                                  ? 'text-brand-300 bg-brand-500/20 border-brand-500/20'
                                  : score >= 50
                                  ? 'text-slate-300 bg-slate-500/20 border-slate-500/20'
                        ...
                        <p className="text-xs text-slate-400 mb-4">
                          {source.authors[0]} et al. ({source.year}) • {source.venue}
                        </p>
                        <button
                          onClick={() => {
                            if (!activeAnalysis.refId) return;
                  ...
                  >
                    ...
                    <span
                      className={`font-bold px-2 py-0.5 rounded ${
                        val > 70 ? 'bg-brand-500/10 text-brand-400' : val > 50 ? 'bg-slate-500/10 text-slate-200' : 'bg-red-500/10 text-red-400'
                      }`}
                    >
                      {val.toFixed(2)}
                    </span>
        ...
        <>
          <div className="p-6 bg-slate-900 text-white relative overflow-hidden shrink-0 border-b border-slate-800">
            <div className="relative z-10">
              ...
              <h3 className="font-display font-bold text-xl leading-tight">Sentence Analysis</h3>
            </div>
          </div>

          <div className="p-6 space-y-6 overflow-y-auto flex-1 custom-scrollbar bg-slate-900/30">
            <div className="glass-card p-5 rounded-2xl bg-slate-800/50 border border-white/5">
              <p className="text-slate-300 italic text-lg leading-relaxed">"{activeAnalysis.text}"</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              {activeAnalysis.triggerPhrase && (
                <div className="p-3 bg-brand-500/10 border border-brand-500/20 rounded-xl mb-2">
                  <span className="text-[10px] text-brand-300 font-bold uppercase tracking-wider block mb-1">Detected Trigger</span>
                  <p className="text-sm font-bold text-brand-100">"{activeAnalysis.triggerPhrase}"</p>
                </div>
              )}
    ...
    <div className="glass-card rounded-3xl overflow-hidden flex flex-col h-full border border-slate-800 relative">
      <div className="p-6 bg-slate-900 border-b border-slate-800">
        ...
        <p className="text-sm text-slate-400 mt-1">Select a category below to jump to findings.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
              ...
              {missingCitations.map((item, i) => (
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-slate-300 line-clamp-2 group-hover:text-white transition-colors">"{item.text}"</p>
                  </div>
                </button>
              ))}
            </div>
        ...
        <section aria-labelledby="gaps-heading">
          <div className="flex items-center justify-between mb-3">
            <h4 id="gaps-heading" className="text-xs font-bold text-red-400 uppercase tracking-wider flex items-center gap-2">
              <Icons.Search className="w-3 h-3" aria-hidden="true" /> Research Gaps ({identifiedGaps.length})
            </h4>
            <div>
              <label htmlFor="gap-filter" className="sr-only">
                Filter research gaps
              </label>
              <input
                id="gap-filter"
                aria-label="Filter research gaps"
                type="text"
                placeholder="Filter gaps..."
                value={gapFilter}
                onChange={(e) => setGapFilter(e.target.value)}
                className="pl-3 pr-3 py-1.5 text-xs bg-slate-800 border border-slate-700 text-white rounded-lg focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all placeholder:text-slate-600"
              />
            </div>
          </div>
          {gapStatus && (
            <div className="mb-2 px-3 py-2 bg-slate-800/60 border border-slate-700 rounded-lg text-[11px] text-slate-300" role="status">
              {gapStatus}
            </div>
          )}
          {identifiedGaps.length > 0 ? (
            <ul role="list" className="space-y-2">
              {identifiedGaps
                .filter((item) => item.text.toLowerCase().includes(gapFilter.toLowerCase()))
                .map((item) => {
                  const isLoading = !!gapLoading[item.idx];
                  const hasError = !!gapErrors[item.idx];
                  const recs = gapRecommendations[item.idx] || [];
                  return (
                    <li key={item.idx} role="listitem" className="rounded-xl border border-red-500/10 bg-red-500/5">
                      <div className="p-3">
                        <p className="text-xs text-slate-300 leading-snug">
                          Marker: "{item.triggerPhrase || 'gap'}"
                        </p>
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                          <button
                            type="button"
                            aria-label={`Jump to sentence ${item.idx + 1}`}
                            onClick={() => {
                              const element = document.querySelectorAll('.max-w-3xl span')[item.idx];
                              element?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                              setActiveAnalysis({
                          ...
                          <button
                            type="button"
                            aria-label={`Find sources for sentence ${item.idx + 1}`}
                            disabled={isLoading}
                            onClick={async () => {
                              setGapErrors((prev) => ({ ...prev, [item.idx]: '' }));
                              setGapLoading((prev) => ({ ...prev, [item.idx]: true }));
                              setGapStatus('Finding sources to address the gap...');
                              try {
                                const sources = await finderService.findSourcesForGap(item.text);
                                setGapRecommendations((prev) => ({ ...prev, [item.idx]: sources }));
                                setGapStatus(
                                  sources.length === 0
                                    ? 'No suitable sources found.'
                                    : `Found ${sources.length} recommended source${sources.length > 1 ? 's' : ''}.`
                                );
                              } catch {
                                setGapErrors((prev) => ({ ...prev, [item.idx]: 'Failed to find sources. Please try again.' }));
                                setGapStatus('');
                              } finally {
                                setGapLoading((prev) => ({ ...prev, [item.idx]: false }));
                              }
                            }}
                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 ${
                              isLoading ? 'bg-slate-800 cursor-not-allowed opacity-60' : 'bg-brand-600 hover:bg-brand-500'
                            } text-white text-xs font-bold rounded-lg border border-brand-500/30 transition-colors`}
                          >
                            {isLoading ? (
                              <Icons.RefreshCw className="w-3.5 h-3.5 animate-spin" aria-hidden="true" />
                            ) : (
                              <Icons.Search className="w-3.5 h-3.5" aria-hidden="true" />
                            )}
                            {isLoading ? 'Searching...' : 'Find Sources'}
                          </button>
                        </div>
                        {hasError && (
                          <div className="mt-2 px-3 py-2 bg-red-500/10 border border-red-500/20 text-red-400 text-[11px] rounded-lg" role="alert">
                            {gapErrors[item.idx]}
                          </div>
                        )}
                        {recs.length > 0 && (
                          <div className="mt-3 space-y-2">
                            {recs.map((ref, j) => (
                              <div key={ref.id || j} className="p-3 rounded-lg bg-slate-900 border border-slate-800">
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    <p className="text-xs font-bold text-slate-200 leading-snug">{ref.title}</p>
                                    <p className="text-[11px] text-slate-500 mt-0.5">
                                      {ref.authors[0]} et al. {ref.year ? `(${ref.year})` : ''}
                                    </p>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    {ref.doi && (
                                      <button
                                        ...
                                        className="inline-flex items-center gap-1.5 px-2 py-1 text-[11px] font-bold rounded border border-brand-500/20 bg-brand-500/10 text-brand-300 hover:bg-brand-500/20 transition-colors"
                                        aria-label="Open DOI"
                                      >
                                        Open <Icons.ExternalLink className="w-3 h-3" aria-hidden="true" />
                                      </button>
                                    )}
                                    <button
                                      type="button"
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        try {
                                          const bibtex = finderService.generateBibTeX(ref as any);
                                          if (navigator?.clipboard?.writeText) {
                                            navigator.clipboard.writeText(bibtex).catch(() => {});
                                          }
                                        } catch {}
                                      }}
                                      className="inline-flex items-center gap-1.5 px-2 py-1 text-[11px] font-bold rounded border border-slate-500/30 bg-slate-700/30 text-slate-200 hover:bg-slate-700 transition-colors"
            ...
            <div
              ...
              role="button"
              aria-label="Learn how research gaps are detected"
            >
              <span className="flex-shrink-0 w-6 h-6 rounded-full bg-brand-500/20 text-brand-400 flex items-center justify-center text-[10px] font-bold mt-0.5 border border-brand-500/20 group-hover:scale-110 transition-transform">
                ?
              </span>
    ...
    <div className="flex flex-col lg:flex-row h-[85vh] gap-8 animate-fade-in">
      ...
      <div className="flex-1 glass-card rounded-3xl flex flex-col overflow-hidden relative group border border-slate-800">
        ...
        <div className="p-5 border-b border-slate-800 bg-slate-900 flex justify-between items-center sticky top-0 z-20">
          <h2 className="font-display font-bold text-white flex items-center gap-3 text-lg">
