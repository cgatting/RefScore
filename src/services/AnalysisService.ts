  ...
  public async analyze(manuscriptText: string, bibText: string): Promise<AnalysisResult> {
    ...
    if (dois.length > 0) {
      ...
      for (const ref of references) {
        if (!ref.doi) continue;
        const key = OpenAlexService.normalizeDoi(ref.doi);
        const oaPaper = oaData[key];
        if (oaPaper) {
          ...
          const venue = oaPaper.primary_location?.source?.display_name || (oaPaper as any).host_venue?.display_name;
          ...
          if (!ref.title && oaPaper.display_name) {
            ref.title = oaPaper.display_name;
          }
          ...
          try {
            const crRes = await fetch(`https://api.crossref.org/works/${encodeURIComponent(key)}`);
            if (crRes.ok) {
              const cr = await crRes.json();
              const msg = cr.message || {};
              if (!ref.title && Array.isArray(msg.title) && msg.title[0]) {
                ref.title = msg.title[0];
              }
              if ((!ref.authors || ref.authors.length === 0) && Array.isArray(msg.author)) {
                ref.authors = msg.author.map((a: any) => [a.given, a.family].filter(Boolean).join(' ')).filter((s: string) => s);
              }
              ...
              if (!ref.year) {
                ...
                if (Array.isArray(parts) && parts[0]) {
                  ref.year = Number(parts[0]);
                }
              }
              if ((!ref.abstract || ref.abstract.length < 50) && typeof msg.abstract === 'string') {
                ref.abstract = msg.abstract;
              }
              if (typeof msg['is-referenced-by-count'] === 'number') {
                ref.citationCount = msg['is-referenced-by-count'];
              }
            }
          } catch {}
        }
