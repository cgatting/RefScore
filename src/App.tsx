...
function App() {
  ...
  const [deepSearchMaxResults, setDeepSearchMaxResults] = useState<number>(20);
      ...
      setTimeout(async () => {
          try {
              ...
              if (deepSearchFlag) {
                ...
                }, { maxResults: deepSearchMaxResults });
  ...
  return (
    <div className="min-h-screen font-sans text-white selection:bg-brand-500/30 selection:text-brand-100 overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3 cursor-pointer group" onClick={handleReset}>
              <div className="bg-black p-3 rounded-2xl shadow-lg shadow-brand-500/30 ring-2 ring-brand-500/30 group-hover:ring-brand-500/50 group-hover:shadow-brand-500/50 group-hover:scale-105 transition-all duration-300">
                <img src={logoUrl} alt="RefScore logo" className="w-8 h-8 rounded-sm" />
              </div>
              <span className="font-display font-extrabold text-2xl tracking-tight text-white group-hover:text-brand-400 transition-colors">
                RefScore
              </span>
            </div>
               ...
               <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-1 bg-slate-900/60 backdrop-blur-sm p-1.5 rounded-xl border border-white/10 shadow-inner">
                <button 
                   ...
                   className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${
                     activeTab === 'dashboard' 
                       ? 'bg-brand-600 text-slate-950 shadow-sm ring-1 ring-brand-500/30' 
      ...
      <main className="pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-screen flex flex-col">
        ...
        {appState === AppState.IDLE && (
          <div className="animate-fade-in-up w-full max-w-5xl mx-auto flex-1 flex flex-col justify-center">
            ...
            {/* Upload Section */}
            <div className="glass-card rounded-3xl p-2 md:p-3 shadow-2xl shadow-black/30 ring-1 ring-white/10">
               <div className="bg-slate-900/50 rounded-2xl border border-slate-700/50 overflow-hidden">
                 <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-700/50">
                    ...
                    <div className="p-8 md:p-12 hover:bg-slate-800/40 transition-colors duration-500">
                      ...
                      {autoGenerateBib && (
                        <div className="mt-3 flex items-center justify-between p-3 rounded-xl border border-slate-700/60 bg-slate-900/40">
                          <label className="text-sm font-bold text-slate-300 mr-3">Max Sources</label>
                          <input
                            type="number"
                            min={1}
                            max={500}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-24 opacity-80">
               {[
                 ...
                 <div key={i} className="flex flex-col items-center text-center gap-4 group cursor-default">
                  ...
                  <h3 className="font-display font-bold text-lg text-white group-hover:text-brand-400 transition-colors">{feature.title}</h3>
